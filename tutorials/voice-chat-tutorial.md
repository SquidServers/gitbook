...
- Link the bedrock tunnel to your server so it auto starts and shuts off with the server
  > Note bedrock tunnel ip will always show up as unreachable in the dashboard, this is normal
...
- Run both the Java tunnel and the Bedrock tunnel at the same time (if linked to server will auto start)
